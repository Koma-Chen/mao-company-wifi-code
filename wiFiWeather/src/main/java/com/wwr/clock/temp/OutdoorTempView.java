...
public class OutdoorTempView extends View {
    ...
    // 每一大行之间的距离
    int YB2GAP = (E2Y - S2Y - (height * 1 / 90)) / 5;
    // 每一小行之间的距离
    int YS2GAP = YB2GAP / 5;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < 7; i++) {
            if (i != 6) {
                if (isC) {
                    if (i == 0) {
                        canvas.drawText("" + 20 * (i - 2), SX * 2 / 10, E1Y
...
//                                - (YB2GAP * i) + 10, blackPAint);
                            ...
                            canvas.drawText("" + (35 * i - 40), SX * 4 / 10, E1Y
                                    - (YB2GAP * i) + 10, blackPAint);
                        }
                    ...
                    if (isC) {
                        ...
                        canvas.drawText("" + (35 * (i) - 40), SX * 2 / 10, E1Y
                                - (YB2GAP * i) + 10, blackPAint);
                    }
                }
            ...
            for (int j = Constant.mlistOutdoor.size() - 1; j >= 0; j--) {
                ...
                float Y = (float) (E1Y - (Constant.mlistOutdoor.get(j).getTempC() + 40)
                        * GAPY);
                /**
                 * 由于我是接手开发的，作为实习生的我诚惶诚恐的，老实说这个自定义View让我自己写我写不出来，但是前辈们写出来了我好歹能看得懂能修改就好；
                 * 前辈们开发的时候室外温度的华氏度显示是从-20到100，比较规律好画，但是现在需求是华氏度改成-40到158，这就蛋疼了；
...
//                float YF = (float) (E1Y - ((Constant.mlistOutdoor.get(j).getTempF()+40) / 35)*YB2GAP);
                float YF = (float) (E1Y - ((Constant.mlistOutdoor.get(j).getTempF()+40D) / 35D)*YB2GAP);
                float YH = (float) (E2Y - (Constant.mlistOutdoor.get(j).getHumidity())
                        * GAPY);
                int X = EX - 25 - (contants * GAP);
                ...
                while (true) {
                    try {
                        if (gt == null) {
                            ...
                            gt.getTemp(strOutDoor, ID_FROM_OUTDOOR);
                            postInvalidate();
                        }
