...
public class TempView extends View {
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int j = Constant.mlist.size() - 1; j >= 0; j--) {
			// X的变化坐标
			float Y = (float) (E1Y - (Constant.mlist.get(j).getTempC() + 20)
					* GAPY);
			float YF = (float) (E1Y - (Constant.mlist.get(j).getTempF()) * GAPY);
			float YH = (float) (E2Y - (Constant.mlist.get(j).getHumidity())
					* GAPY);
			int X = EX - 25 - (contants * GAP);

			if (isC) {
				if (Constant.mlist.get(j).getTempC() != -99) {
					// 绘制摄氏度的点
				...
				// 绘制线 各点之间的线
				if (j == (Constant.mlist.size() - 1)) {// 第一个点就是开始的地方
					// 开始的地方
					if (Constant.mlist.get(j).getTempC() != -99.0) {
						linePath.moveTo(X, Y);
					}
				} else {
					if (Constant.mlist.get(j).getTempC() != -99.0) {
						if (Constant.mlist.get(j + 1).getTempC() != -99.0) {
							linePath.lineTo(X, Y); //
						} else {
							linePath.moveTo(X, Y);
						}
				...
				// 绘制华氏度的点
				if (Constant.mlist.get(j).getTempF() != -99.0) {
					canvas.drawCircle(X,// 起点X
							YF,// 起点Y
							2,// 半径
							paintData);
				...
				if (j == (Constant.mlist.size() - 1)) {// 第一个点就是开始的地方
					...
					if (Constant.mlist.get(j).getTempF() != -99.0) {
						if (Constant.mlist.get(j + 1).getTempF() != -99.0) {
			...
			canvas.drawText(Constant.mlist.get(j).getTime(), X - 25, E1Y
					+ height * 1 / 48 + YS2GAP, bluePAint);

			// 绘制湿度的点
			if (Constant.mlist.get(j).getHumidity() != -99) {
				canvas.drawCircle(X,// 起点X
						YH,// 起点Y
						2,// 半径
						paintF);
			...
			if (j == (Constant.mlist.size() - 1)) {// 第一个点就是开始的地方
				...
				if (Constant.mlist.get(j).getHumidity() != -99) {
					if (Constant.mlist.get(j + 1).getTempF() != -99) {
			...
			canvas.drawText(Constant.mlist.get(j).getTime(), X - 25, E2Y
			...
			if (isC) {
				canvas.drawText(
						context.getString(R.string.indoor_temperature)
								+ Constant.mlist.get(Constant.mlist.size() - 1)
										.getStrC() + " ℃", TX, TY, pt);
			} else {
				canvas.drawText(
						context.getString(R.string.indoor_temperature)
