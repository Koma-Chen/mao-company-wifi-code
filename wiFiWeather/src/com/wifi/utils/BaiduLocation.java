				...
				if(location.getCity()!=null){//定位成功但是没有城市   直接不管
					...
					if("中国".equals(location.getCountry())){	//定位结果是国内
						...
						for (City c : MyApplication.CN_LIST) {
							if ((c.getName().indexOf(afterCity)!=-1||afterCity.indexOf(c.getName())!=-1)) {
								LoactionCitys.add(c);
							}
						}
						...
						if(LoactionCitys.size()==0){		//无匹配
							...
							new Thread(){
								public void run() {
									new SendCity2Service().send(city);
								};
							}.start();
						}else{		//有匹配
							lrb.setResltCode(1);
						...
						for (City c : MyApplication.FORE_LIST) {
							if ((afterCity.indexOf(toLower(c.getPinyi().replace(" ", "")))!=-1)||(toLower(c.getPinyi().replace(" ", "")).indexOf(afterCity)!=-1)) {
								LoactionCitys.add(c);
							}
						}
						if (LoactionCitys.isEmpty()) { // 定位到城市且本地库没有数据
							...
							try {
								...
								if ("{".equals(String.valueOf(sb1.toString().charAt(0)))) {
									JSONObject object1 = new org.json.JSONObject(sb1.toString().trim());
									...
									if (count == 0) { // 雅虎查询不到该城市
										...
										JSONObject channel = results.getJSONObject("channel"); // 具体数据
